...
export const getEx = async (id: string) => {
  try {
    const data: any =
      await sql`SELECT * FROM bubu WHERE _id=${id}`;
    const { rows: ex } = data;
    return ex;
  } catch (error) {
...
export const getExs = async () => {
  try {
    const data: any = await sql`SELECT * FROM bubu`;
    const { rows: exs } = data;
    return exs;
  } catch (error) {
    console.log(error);
...
export const createEx = async (formValues: IBlog) => {
  try {
    const result = await sql`INSERT INTO bubu (_id, avatar, name, description, age, CreatedAt) VALUES (${
      formValues._id
...
export const updateEx = async (blog: IBlog) => {
  try {
    await sql`UPDATE bubu SET name=${blog.name},avatar=${blog.avatar},age=${blog.age},description=${blog.description} WHERE _id = ${blog._id}`;
    revalidatePath("/home");
    return "success";
  } catch (error) {
    console.log(error);
...
export const changeDisLikeEx = async (id: string, dislike: boolean) => {
  try {
    await sql`UPDATE bubu SET dislike = ${dislike} WHERE _id = ${id}`;
    revalidatePath("/home");
    return "success";
  } catch (error) {
    console.log(error);
...
export const deleteEx = async (id: string) => {
  try {
    await sql`DELETE FROM bubu WHERE _id=${id};`;
    revalidatePath("/home");
    return "Success";
  } catch (error) {
    console.log(error);
